...
t('ref - added keys', function() {
	...
	t('!', store.act({v:{aa: {bb:{cc:{}, c: 'c'}, b: 'b'}, a:'a'}}), 'no errors')
...
t('ref - del keys', function() {
	...
	store.act({v:{aa: {bb:{cc:{}, c: 'c'}, b: 'b'}, a:'a'}})
...
t('db - query', function() {
	var store = new Store(null)
	store.act({v:{data: {list: [1,0]}, view:{}, acts:{}, done:[]}})

	store.on('data/list', function(v) {
		store.act({p:'view/sort', v:v.slice().sort()})
	})
	store.on('acts/push', function(v) {
		store.act({p:'data/list', v:store.get('data/list').concat(v)})
	})
...
t('store - errors', function() {
	...
	t('===', store.act({p:'a/b', v:9}) instanceof Error, true)
	t('===', store.act({p:'a/b'}) instanceof Error, true)
	var res = store.act({p:'a/b', v:9})
